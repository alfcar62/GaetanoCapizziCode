...
/**
*   carica un quadrante 3*3 sul sudoku con numeri da 1 a 9
*   params:
*       @param int sudoku[][] : output : matrice su cui riempire il quadrante
*       @param int quadrante : input : quadrante da riempire
*       @return void
*/
void caricaQuad(int sudoku[][DIM],int quadrante)
{
    ...
    int riga = (quadrante/3) * 3, col = (quadrante%3) * 3;
    int num = 1,index,numAvai; // numAvai = numbers available
    bool giusto;
    ...
    for(int i = 0;i < 3;i++)
    {
        for(int j = 0;j < 3;j++)
        {
            ...
            while(giusto == false)
            {
                ...
                if(numAvai == 0) // se nessun numero da 1 a 9 può stare in questa posizione allora mette 0 e va alla prossima posizione
                {
                    num = 0;
                    giusto = true;
                }
                ...
                {
                    num = pickRand(nums,numAvai,&index);
                    giusto = checkNum(sudoku,num,riga+i,col+j,quadrante);
            ...
            sudoku[riga+i][col+j] = num;

            clearTriedNums(nums,sizeof(nums)/4);
        }
    }
}
