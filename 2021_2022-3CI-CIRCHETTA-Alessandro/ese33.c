...
CONSEGNA: Calcolare il codice fiscale: step 1: Calcolo del cognome
*/
...
int input(char stringa[], char var[])
{
    bool errore;
    int i = 0;
...
/*void divisioneconsonantivocali(char stringa[], char vocali[], char consonanti[])
{
    ...
    bool errore;
        ...
        if( stringa[i] != 'A' && stringa[i] != 'E' && stringa[i] != 'I' && stringa[i] != 'O' && stringa[i] != 'U' && stringa[i] != ' '&& stringa[i] != '\'')
        {
